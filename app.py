...
if "Predictor" in page:
    ...
    if uploaded:
        ...
        if st.button("🧮 Predict Batch"):
            ...
            st.download_button("⬇️ Download Results CSV", out.to_csv(index=False), "batch_predictions.csv", "text/csv")
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if st.session_state["batch_results"] is None:
        ...
        if "pH" in df.columns and "tds" in df.columns:
            st.plotly_chart(px.scatter(df, x="pH", y="tds", color="prediction", size="risk_score",
                                       hover_data=["turbidity", "temp"], title="pH vs TDS"), use_container_width=True)
